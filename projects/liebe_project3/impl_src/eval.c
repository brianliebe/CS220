...
double compute_access_time() {
    ...
    */
    double readAndWrite = writes_count + reads_count;

    double timeForFaults = pagefaults_count * DISK_IO_TIME;
    double timeForHits = tlbhits_count * TLB_ACCESS_TIME;
    double timeForMisses = (readAndWrite - tlbhits_count) * 2 * MEM_ACCESS_TIME;

    return timeForMisses + timeForHits + timeForFaults;
